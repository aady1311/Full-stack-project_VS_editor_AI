  ...
  const loadPlayground = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Template directory missing — start with empty workspace
        if (res.status === 404) {
          setTemplateData({ folderName: "Root", items: [] });
          toast.info("No starter template found. Starting with empty workspace.");
          return;
        }
        throw new Error(errData.error || `Failed to load template: ${res.status}`);
      }
